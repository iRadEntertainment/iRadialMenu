...
@export var base_circle_fill: bool = false
...
func _set_children_modulate(value: Color) -> void:
	children_modulate = value
	for child in get_children():
		if child is Control:
			child.modulate = children_modulate
...
func set_temporary_selection(value: int = -2) -> void:
	...
	for child in get_children():
		if child is Control:
			child.modulate = hover_child_modulate if child.get_index() == value else children_modulate
...
func get_selected_child() -> Node:
	return get_child(selection + (1 if first_in_center else 0))
...
func _draw() -> void:
	...
	if base_circle_fill:
		...
		var point_angle: float = (TAU / float(arc_detail)) 
		for i: int in arc_detail + 1:
			var dir: Vector2 = Vector2.from_angle(point_angle * i)
			var p_in: Vector2 = offset + dir * arc_inner_radius
			var p_out: Vector2 = offset + dir * radius
			inner.append(p_in)
			outer.append(p_out)
		outer.reverse()
		var poly_top: PackedVector2Array = inner.slice(arc_detail/2) + outer.slice(0, arc_detail/2 + 1)
		var poly_bot: PackedVector2Array = inner.slice(0, arc_detail/2 + 1) + outer.slice(arc_detail/2)
		draw_polygon(poly_top, [color])
		draw_polygon(poly_bot, [color])
	...
	if (mouse_pos != -Vector2.ONE):
		...
		if selection != prev_selection:
			if selection >= 0:
				for child in get_children():
					if child is Control:
						child.modulate = hover_child_modulate if child.get_index() == selection else children_modulate
